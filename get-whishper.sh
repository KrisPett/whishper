...
if [ "$gpu" = true ] ;then
    curl -o docker-compose.yml https://raw.githubusercontent.com/KrisPett/whishper/main/docker-compose.gpu.yml > /dev/null 2>&1
else
    curl -o docker-compose.yml https://raw.githubusercontent.com/KrisPett/whishper/main/docker-compose.yml > /dev/null 2>&1
fi
sleep 1
...
if [ -f .env ]; then
    ...
    if [ "$answer" != "${answer#[Yy]}" ] ;then
        ...
        cp .env .env.backup
        curl -o .env https://raw.githubusercontent.com/KrisPett/whishper/main/example.env > /dev/null 2>&1
        sleep 1
    fi
    echo ""
else
